defmodule SyntheticManagerWeb.CoreComponents do
  @moduledoc """
  ...
  def open_menu(assigns) do
    ~H"""
    <button
      phx-click={JS.set_attribute({"aria-hidden", "false"}, to: "header .mobile")}
  ...
  def slugify(string, suffix) do
    slugify(string) <> "-" <> suffix
  end
  ...
  def icon_link(assigns) do
    ...
    <div
      ...
      <div class="flex-auto">
        <a href={@path} class="block font-semibold text-gray-900 relative">
          <%= render_slot(@name) %>
          <span class="absolute inset-0"></span>
        </a>
        <p :if={@tagline != []} class="tagline mt-1 text-gray-600"><%= render_slot(@tagline) %></p>
      </div>
    </div>
    """
  end
  ...
  def dropdown_menu_toggle(assigns) do
    ~H"""
    <button
      type="button"
      phx-click={JS.set_attribute({"aria-expanded", "true"}, to: ".#{slugify(@label, "dropdown")}")}
      class="open-dropdown toggle-dropdown"
      >
      <%= @label %>
    ...
    <button
    type="button"
    phx-click={JS.set_attribute({"aria-expanded", "false"}, to: ".#{slugify(@label, "dropdown")}")}
    class="close-dropdown toggle-dropdown"
    >
    <%= @label %>
    <svg class="h-5 w-5 flex-none text-gray-400" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
      <path fill-rule="evenodd" d="M5.23 7.21a.75.75 0 011.06.02L10 11.168l3.71-3.938a.75.75 0 111.08 1.04l-4.25 4.5a.75.75 0 01-1.08 0l-4.25-4.5a.75.75 0 01.02-1.06z" clip-rule="evenodd" />
  ...
  def dropdown_menu(assigns) do
    ~H"""
    <div class={" dropdown-menu #{slugify(@label, "dropdown")}"} aria-expanded="false">
    <.dropdown_menu_toggle label={@label} />
    <div
      phx-click-away={JS.set_attribute({"aria-expanded", "false"}, to: "nav .#{slugify(@label, "dropdown")}")}
      class="dropdown"
      >
      <%= render_slot(@section) %>
    </div>
  ...
  def header_bar(assigns) do
    ...
    <header class="bg-white">
      ...
      <div class="mobile lg:hidden" role="dialog" aria-modal="true" aria-hidden="true">
        <!-- Background backdrop, show/hide based on slide-over state. -->
        <div class="fixed inset-0 z-10"></div>
        <div
          phx-click-away={JS.set_attribute({"aria-hidden", "true"}, to: "header .mobile")}
          ...
          <div class="flex items-center justify-between">
            ...
            <button
              phx-click={JS.set_attribute({"aria-hidden", "true"}, to: "header .mobile")}
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(SyntheticManagerWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(SyntheticManagerWeb.Gettext, "errors", msg, opts)
    end
  end
    ...
    <div>
      ...
      <div class="mt-1 relative">
        <select name={@field.name} id={@field.id}
          ...
          <%= for user <- @users do %>
            <.custom_option user={user} selected={false} />
          <% end %>
        </select>
